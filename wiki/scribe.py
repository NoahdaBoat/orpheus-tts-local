"""Wiki scribe agent: digests a chat into an Obsidian vault.
...
from lm import AgentError, AgentResult, run_tool_loop
from wiki.tools import all_tool_schemas, dispatch_tool, read_tool_schemas
from wiki.vault import WikiVault
...
def build_chat_messages_for_lm(
    ...
    *,
    ...
    if wiki_enabled:
        system = (
            (system + "\n" + MAIN_WIKI_ADDENDUM).strip()
            if system
            else MAIN_WIKI_ADDENDUM.strip()
        )
    if system:
    ...
    for item in chat.get("messages") or []:
        ...
        if (
            role == "assistant"
            and item.get("status") in ("error", "cancelled")
            and not content
        ):
            continue
...
def run_scribe(
    *,
    ...
    pages_before = {
        item["path"] for item in vault.list_notes(limit=200).get("notes", [])
    }
    touched: list[str] = []
